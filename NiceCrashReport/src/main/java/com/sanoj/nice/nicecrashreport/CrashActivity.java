...
public final class CrashActivity {
    ...
    private static final String EXTRA_CONFIG = "com.sanoj.nice.nicecrashreport.EXTRA_CONFIG";
    private static final String EXTRA_STACK_TRACE = "com.sanoj.nice.nicecrashreport.EXTRA_STACK_TRACE";
    private static final String EXTRA_ACTIVITY_LOG = "com.sanoj.nice.nicecrashreport.EXTRA_ACTIVITY_LOG";

    //General constants
    private static final String INTENT_ACTION_ERROR_ACTIVITY = "com.sanoj.nice.nicecrashreport.ERROR";
    private static final String INTENT_ACTION_RESTART_ACTIVITY = "com.sanoj.nice.nicecrashreport.RESTART";
    private static final String CRASH_HANDLER_PACKAGE_NAME = "com.sanoj.nice.nicecrashreport.";
    ...
    private static NiceCrash config = new NiceCrash();
                    ...
                    Thread.setDefaultUncaughtExceptionHandler((thread, throwable) -> {
                        if (config.isEnabled()) {
                            ...
                            if (hasCrashedInTheLastSeconds(application)) {
                                ...
                                } else if (config.getBackgroundMode() == NiceCrash.BACKGROUND_MODE_SHOW_CUSTOM || !isInBackground) {
                                ...
                                } else if (config.getBackgroundMode() == NiceCrash.BACKGROUND_MODE_CRASH) {
                                    if (oldHandler != null) {
                                        oldHandler.uncaughtException(thread, throwable);
                                        return;
                                    }
    ...
    public static NiceCrash getConfigFromIntent(@NonNull Intent intent) {
        NiceCrash config = (NiceCrash) intent.getSerializableExtra(CrashActivity.EXTRA_CONFIG);
    ...
    private static void restartApplicationWithIntent(@NonNull Activity activity, @NonNull Intent intent, @NonNull NiceCrash config) {
    ...
    public static void restartApplication(@NonNull Activity activity, @NonNull NiceCrash config) {
        Intent intent = new Intent(activity, config.getRestartActivityClass());
        restartApplicationWithIntent(activity, intent, config);
    }

    public static void closeApplication(@NonNull Activity activity, @NonNull NiceCrash config) {
        if (config.getEventListener() != null) {
    ...
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    @NonNull
    public static NiceCrash getConfig() {
        return config;
    }

    @RestrictTo(RestrictTo.Scope.LIBRARY)
    public static void setConfig(@NonNull NiceCrash config) {
